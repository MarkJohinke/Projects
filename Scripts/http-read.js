...
async function main() {
  ...
  if (!target || !remotePath) {
    console.error("Usage: node scripts/http-read.js --target <dev|personal|yoga> --path <remotePath> [--url http://host:8765] [--token <token>]");
    process.exit(2);
  }
  const url = new URL("/tools/read", baseUrl);
