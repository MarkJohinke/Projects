...
async function main(){
  ...
  const target=args.target||args.t; const dir=args.path||args.p;
  const token=args.token||process.env.API_TOKEN||"";
  if(!target||!dir){console.error("Usage: node scripts/dsm-list.js --target <dev|personal|yoga> --path <folder> [--url http://host:8765] [--token <token>]");process.exit(2);}  
  const url=new URL("/dsm/list", base);
  const payload={target:pathClean(dir), path:pathClean(dir)};
...
function pathClean(p){return String(p||"").trim();}
