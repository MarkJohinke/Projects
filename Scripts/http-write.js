...
async function main(){
  ...
  if(!target||!remotePath){console.error("Usage: node scripts/http-write.js --target <dev|personal|yoga> --path <remotePath> [--content text] [--encoding utf8|base64] [--url http://host:8765] [--token <token>]");process.exit(2);}  
  const url=new URL("/tools/write", base);
  const payload={target,remotePath,content,encoding};
