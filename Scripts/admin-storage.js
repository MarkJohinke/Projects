...
async function main() {
  ...
  if (!target) {
    console.error("Usage: node scripts/admin-storage.js --target <dev|personal|yoga> [--url http://host:8765] [--token <token>]");
    process.exit(2);
  }
  const url = new URL("/admin/storage-summary", baseUrl);
