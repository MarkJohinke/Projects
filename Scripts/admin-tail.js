...
async function main(){
  ...
  const target=args.target||args.t; const logPath=args.path||args.p; const lines=Number(args.lines||200);
  const token=args.token||process.env.API_TOKEN||"";
  if(!target||!logPath){console.error("Usage: node scripts/admin-tail.js --target <dev|personal|yoga> --path /var/log/messages [--lines 200] [--url http://host:8765] [--token <token>]");process.exit(2);}  
  const url=new URL("/admin/log-tail", base);
  const payload={target,path:logPath,lines};
