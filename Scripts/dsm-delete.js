...
async function main(){
  ...
  const target=args.target||args.t; const recursive=!!args.recursive;
  ...
  if(!target||paths.length===0){console.error("Usage: node scripts/dsm-delete.js --target <dev|personal|yoga> --paths /path/a,/path/b [--recursive] [--url http://host:8765] [--token <token>]");process.exit(2);}  
  const url=new URL("/dsm/delete", base);
  const payload={target,paths,recursive};
